...
export const Timer = ({ focusSubject, clearSubject, onTimerEnd }) => {
  ...
  const [progress, setProgress] = useState(1); // counts down from 1 to 0
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.clearSubjectWrapper}>
        <RoundedButton size={50} title="clr" onPress={clearSubject} />
      </View>
    </View>
  );
};
...
const styles = StyleSheet.create({
  ...
  timingWrapper: { flex: 0.1, flexDirection: 'row', paddingTop: spacing.lg },
  buttonWrapper: {
    flex: 0.3,
    flexDirection: 'row',
    padding: spacing.md,
